...
impl BUS {
    pub fn new(mapper: *mut dyn Mapper, ppu: *mut PPU) -> Self {
        BUS {
            ram: [0; RAM_SIZE],
            mapper,
            ppu,
    ...
    pub fn write(&mut self, addr: u16, value: u8) {
        ...
        match addr {
            ...
            0x4020..=0xFFFF => unsafe { (*self.mapper).write_prg(addr, value); },
            _ => ()
        }
    }

    pub fn read(&mut self, addr: u16) -> u8 { 
        // log(&format!("BUS READ: {}", addr));
        match addr {
            ...
            0x4020..=0xFFFF => unsafe { (*self.mapper).read_prg(addr)},
            _ => 0
        }
    }
}
