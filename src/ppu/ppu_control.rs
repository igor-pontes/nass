...
impl PPUControl {
    ...
    pub fn get_nametable(&self) -> u8 {
        (self.intersects(PPUControl::NAMETABLE2) as u8) << 1 | 
        (self.intersects(PPUControl::NAMETABLE1) as u8)
    }

    pub fn update(&mut self, data: u8, temp: &mut u16) {
        *self = PPUControl::from_bits_retain(data);
        *temp = (( self.get_nametable() as u16 ) << 10) | (*temp & 0xF3FF);
    }

    pub fn is_sprite_size_16(&self) -> bool {
        self.intersects(PPUControl::SPRITE_SIZE_16)
    }

    pub fn get_sprite_pattern_addr(&self) -> u16 {
        // ignored in 8x16 mode
        let right_table = self.intersects(PPUControl::SPRITE_PATTERN_ADDR);
        if right_table { 0x1000 } else { 0 }
    }

    pub fn get_background_pattern_addr(&self) -> u16 {
        let right_table = self.intersects(PPUControl::BACKGROUND_PATTERN_ADDR);
        if right_table { 0x1000 } else { 0 }
    }
    
    pub fn generate_nmi(&self) -> bool {
        self.intersects(PPUControl::GENERATE_NMI)
    }
}
